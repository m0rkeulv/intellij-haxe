...
import com.intellij.plugins.haxe.lang.psi.indexes.unified.specialized.HaxeClassInheritanceUnifiedIndex;
import com.intellij.plugins.haxe.lang.psi.indexes.unified.specialized.HaxeTypedefInheritanceUnifiedIndex;
...
public class HaxeInheritanceDefinitionsSearcher extends QueryExecutorBase<PsiElement, DefinitionsScopedSearch.SearchParameters> {
  ...
  static private void processInheritors(final String qName, final PsiElement context, final Processor<? super PsiElement> consumer) {
    ...
    while (!namesQueue.isEmpty()) {
      ...
      candidates.addAll(HaxeClassInheritanceUnifiedIndex.getBySuper(simpleName, project, scope));
      candidates.addAll(HaxeTypedefInheritanceUnifiedIndex.getBySuper(simpleName, project, scope));
