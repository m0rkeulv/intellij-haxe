...
public class HaxeIndexUtil {
  public static int BASE_INDEX_VERSION = 136;
  ...
  public static boolean fileBelongToPlatformSpecificStd(@NotNull StubBase<?> stub) {
    PsiFile psiFile = stub.getContainingFileStub().getPsi();
    return fileBelongToPlatformSpecificStd(psiFile);
  }
  ...
  public static boolean isStdTypeFile(@Nullable PsiFile file) {
    if (file == null)  return false;
    HaxeProjectModel project = HaxeProjectModel.fromElement(file);
    if (project.getSdkRoot().contains(file)) {
      return file.getVirtualFile().getName().equals("StdTypes.hx");
    }
    return false;
  }
