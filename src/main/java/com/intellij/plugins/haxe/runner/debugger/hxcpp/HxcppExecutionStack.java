 ...
 */
final class HxcppExecutionStack extends XExecutionStack {
  private final HxcppDebugProcess process;
  private final int threadId;
  private final @Nullable List<HxcppStackFrame> eagerFrames;
  ...
  HxcppExecutionStack(HxcppDebugProcess process, int threadId, String displayName,
  ...
  private List<HxcppStackFrame> toFrames(List<StackFrame> dapFrames) {
    return dapFrames.stream().map(frame -> new HxcppStackFrame(process, frame)).toList();
  }
