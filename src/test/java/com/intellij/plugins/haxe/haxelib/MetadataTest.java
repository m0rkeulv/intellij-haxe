...
public class MetadataTest {
  ...
  private JsonElement createTestElement() {
    ...
    el.addProperty(HaxelibMetadata.NAME, NAME_DATA);
    el.addProperty(HaxelibMetadata.URL, URL_DATA);
    el.addProperty(HaxelibMetadata.LICENSE, LICENSE_DATA);

    JsonArray tags = new JsonArray();
    tags.add(new JsonPrimitive(TAGS_DATA1));
    tags.add(new JsonPrimitive(TAGS_DATA2));
    el.add(HaxelibMetadata.TAGS, tags);

    el.addProperty(HaxelibMetadata.DESCRIPTION, DESCRIPTION_DATA);
    el.addProperty(HaxelibMetadata.VERSION, VERSION_DATA);
    el.addProperty(HaxelibMetadata.CLASSPATH, CLASSPATH_DATA);
    el.addProperty(HaxelibMetadata.RELEASENOTE, RELEASENOTE_DATA);
