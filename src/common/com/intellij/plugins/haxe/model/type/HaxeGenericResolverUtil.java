...
public class HaxeGenericResolverUtil {
  ...
  public static HaxeGenericResolver generateResolverFromScopeParents(PsiElement element) {
    ...
    boolean isArrayExpression = element instanceof HaxeArrayAccessExpression;
    ...
    if(isCallExpression || isArrayExpression) {
      PsiElement[] children = element.getChildren();
      if(children.length == 1 )appendStatementGenericResolver(HaxeResolveUtil.getLeftReference(children[0]), resolver);
      if(children.length == 2 )appendStatementGenericResolver(HaxeResolveUtil.getLeftReference(children[1]), resolver);
    }
    if (isCastExpression) {
    ...
    appendArrayAccesGenericResolver(element, resolver);
    return resolver;
  }
  ...
  @Nullable static HaxeGenericResolver appendArrayAccesGenericResolver(PsiElement element, @NotNull HaxeGenericResolver resolver) {
    if (null == element) return resolver;
        ...
        if (result.getHaxeClass()!= null) {
          ...
          for (HaxeMethodModel method : methods) {
            if (method.isArrayAccessor()) {
              // TODO  make resolver method specific (parameter could in theory be a Type parameter, so child(1) might also be relevant)
              resolver.addAll(result.getGenericResolver());
             break;
            }
          }
